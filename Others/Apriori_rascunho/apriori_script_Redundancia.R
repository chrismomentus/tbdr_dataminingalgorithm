...
for (i in 1:1) {
  rdnum <- sample(1:100, 2, replace=FALSE)
  print(rdnum[1])
  print(rdnum[2])
  #seed1 = 110
  #seed2 = 122
  
  coincedente = function_valida(df, rdnum[1], rdnum[2])
  ...
  for (k in 2:run) {
    ...
    rdnum <- sample(101:200, 2, replace=FALSE)
    print(rdnum[1])
    print(rdnum[2])
    
    Regras_valida1 <- function_valida(df, rdnum[1], rdnum[2])
...
write.csv(coincedente, "coincedente.csv")
#-----------------------
#-------------------------

coincedentes <- read.csv(file="coincedente.csv", header = TRUE, sep =",")
...
write.csv(desfechos00, "desf0Cura_coincedente.csv")
write.csv(desfechos11, "desf1Abandono_coincedente.csv")
write.csv(desfechos22, "desf2Falencia_coincedente.csv")
write.csv(desfechos33, "desf3Obito_coincedente.csv")
